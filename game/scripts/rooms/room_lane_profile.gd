...
func get_lane_z(index: int) -> float:
	if not lane_z_values.is_empty():
		return clampf(lane_z_values[wrapi(index, 0, lane_z_values.size())], z_min, z_max)
	return clampf(get_unfold_center_z() + get_lane_offset(index), z_min, z_max)
...
func pick_enemy_lane(player_unfold_position: Vector2, enemy_index: int, preferred_lane: int = -1) -> int:
	if preferred_lane >= 0:
		return wrapi(preferred_lane, 0, get_lane_count())
...
func clamp_unfold_point(point: Vector2) -> Vector2:
	return Vector2(clampf(point.x, x_min, x_max), clampf(point.y, z_min, z_max))
...
func get_player_restore_position(unfold_position: Vector2) -> Vector2:
	return Vector2(clampf(unfold_position.x, x_min, x_max), get_nearest_platform_y(unfold_position))

func get_enemy_restore_position(unfold_position: Vector2) -> Vector2:
	if enemy_ground_y_values.is_empty():
		return Vector2(clampf(fallback_enemy_relocation.x, x_min, x_max), fallback_enemy_relocation.y)
	return Vector2(clampf(unfold_position.x, x_min, x_max), get_nearest_enemy_ground_y(unfold_position))
